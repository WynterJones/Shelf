      ...
      h("div", { class: "button-container group-container" }, [
        h(
          "button",
          {
            ...
            title: group.name,
            onMouseenter: (e) => showGroupPopout(e, group.id),
        ...
        h(
          "div",
          { class: "group-popout", id: `group-${group.id}` },
          groupApps.map((app) => {
            const isImage =
              app.icon &&
              (app.icon.startsWith("http") ||
...
function showGroupPopout(event, groupId) {
  const popout = document.getElementById(`group-${groupId}`);
  if (popout) {
...
function hideGroupPopout() {
  document.querySelectorAll(".group-popout").forEach((p) => {
